...
This document freezes the Phase A Runtime HTTP/SSE contract that Phase B BFF
and product UI code may consume. The contract is backed by the shared TypeScript
schemas in `packages/shared/src` and the runtime routes in
...
before passing data into UI state.
...
| `tool.failed` | `runId`, `timestamp`, `tool`, `error`, `toolUseId`, `recoverable` |
...
| Method | Path | Status |
|---|---|---|
| `POST` | `/internal/template-build` | Local/test/admin helper; not a product BFF dependency |
...
operation. Promotion remains enforced by runtime tools and gates.
...
The freeze gate passed on 2026-07-08.
