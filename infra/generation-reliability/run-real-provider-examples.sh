...
"${KUBECTL}" --context "${context}" get deployment anydesign-runtime \
  -n "${namespace}" >/dev/null
"${KUBECTL}" --context "${context}" rollout status deployment/anydesign-runtime \
...
node - "${provider_config}" "${cases_file}" <<'NODE'
const fs = require("node:fs");
...
if (!resource?.enabled || resource.kind !== "openai_compatible" || !resource.auth?.secretRef) {
  throw new Error("deepseek-v4-pro is not an enabled credential-backed Provider resource");
}
const policy = config.policies?.find(item =>
  item.candidates?.some(candidate => candidate.modelResourceId === "deepseek-v4-pro"));
if (!policy || policy.limits?.dailyInputTokens < manifest.budget.totalTokens) {
  throw new Error("deepseek-v4-pro daily input-token policy is below the suite safety ceiling");
}
process.stdout.write(
  `Validated governed deepseek-v4-pro Provider resource; suite safety ceiling=${manifest.budget.totalTokens}, provider daily input ceiling=${policy.limits.dailyInputTokens}\n`,
);
NODE

runtime_provider="$("${KUBECTL}" --context "${context}" -n "${namespace}" \
  get deployment anydesign-runtime \
...
work_dir="$(mktemp -d)"
principal_private_key="${work_dir}/principal-private.pem"
principal_public_key="${work_dir}/principal-public.der"
principal_secret_backup="${work_dir}/principal-secret.yaml"
admin_token_file="${work_dir}/runtime-admin-token"
port_forward_log="${work_dir}/port-forward.log"
port_forward_pid=""
runtime_mutated=false

old_input_budget="$("${KUBECTL}" --context "${context}" -n "${namespace}" \
  get deployment anydesign-runtime \
  -o jsonpath='{.spec.template.spec.containers[0].env[?(@.name=="RUNTIME_AGENT_MAX_INPUT_TOKENS")].value}')"
old_output_budget="$("${KUBECTL}" --context "${context}" -n "${namespace}" \
  get deployment anydesign-runtime \
  -o jsonpath='{.spec.template.spec.containers[0].env[?(@.name=="RUNTIME_AGENT_MAX_OUTPUT_TOKENS")].value}')"
old_turn_budget="$("${KUBECTL}" --context "${context}" -n "${namespace}" \
  get deployment anydesign-runtime \
  -o jsonpath='{.spec.template.spec.containers[0].env[?(@.name=="RUNTIME_AGENT_MAX_TURNS")].value}')"
old_tool_budget="$("${KUBECTL}" --context "${context}" -n "${namespace}" \
  get deployment anydesign-runtime \
  -o jsonpath='{.spec.template.spec.containers[0].env[?(@.name=="RUNTIME_AGENT_MAX_TOOL_CALLS")].value}')"
...
cleanup() {
  ...
  if [[ "${runtime_mutated}" == "true" ]]; then
    ...
    "${KUBECTL}" --context "${context}" set env deployment/anydesign-runtime \
      -n "${namespace}" \
...
"${KUBECTL}" --context "${context}" get secret anydesign-runtime-public-principal \
  -n "${namespace}" -o yaml >"${principal_secret_backup}"
"${KUBECTL}" --context "${context}" get secret anydesign-runtime-internal-admin \
...
"${KUBECTL}" --context "${context}" port-forward -n "${namespace}" \
  service/anydesign-runtime "${runtime_port}:8080" >"${port_forward_log}" 2>&1 &
port_forward_pid=$!
...
if rg -n -i '(bearer[[:space:]]+[a-z0-9._-]{20,}|sk-[a-z0-9_-]{12,}|api[_-]?key["[:space:]]*:)' \
