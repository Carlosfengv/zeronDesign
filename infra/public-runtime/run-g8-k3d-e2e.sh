...
"$KUBECTL" delete deployment,service,ingress,networkpolicy,endpointslice -n ws-public-runtime-e2e \
  -l app.kubernetes.io/managed-by=anydesign-work-runtime-controller \
  --ignore-not-found=true --wait=true >/dev/null
"$KUBECTL" delete endpointslice/g8-blocking-old-endpoint -n ws-public-runtime-e2e \
  --ignore-not-found=true --wait=true >/dev/null
