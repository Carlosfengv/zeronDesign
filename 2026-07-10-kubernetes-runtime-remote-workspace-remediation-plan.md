...
| k3d gate 未证明镜像来自当前 checkout | 可能在旧 AnyGen 镜像/旧集群上得到绿色结果 | 镜像必须带 Git SHA，导入目标 k3d，并校验 Pod `imageID`、Runtime commit 和测试 evidence commit 一致 |
...
| 项目 | 结果 |
|---|---|
| k3d cluster | `anygen`, server `1/1` Ready |
...
k3d runner 必须新建或显式选择专用集群，不能复用无法证明来源的 `k3d-anygen` 环境。镜像
...
container config `imageID`。旧 `k3d-anygen`、固定 Pod 名、手工 tunnel、宿主机 Workspace
