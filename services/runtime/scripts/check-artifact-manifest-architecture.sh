...
if ! grep -Eq 'ArtifactResolver::load_for_version' "$ARTIFACT_ROUTES"; then
  fail "ART-003: new artifacts must resolve from the verified manifest"
fi
...
rewrite_hits="$(grep -RIl --include='*.rs' 'rewrite_artifact_html' "$HTTP_DIR" || true)"
if [[ "$rewrite_hits" != "$HTTP_DIR/artifact_legacy.rs" ]]; then
  fail "ART-006: historical framework rewrite must remain isolated in artifact_legacy.rs"
fi
