...
function dockerHubImage(lock, digest = lock.digest) {
  const repository = lock.source.split(":")[0];
  const canonical = repository.includes("/") ? repository : `registry-1.docker.io/library/${repository}`;
...
function build(request) {
  ...
  const result = run("docker", [
    ...
    "--build-arg", `BASE_IMAGE=${dockerHubImage(locks.staticWebBase, request.baseImageDigest)}`,
...
function push({ request, image: built }) {
  const root = releaseDir(request.releaseId);
  const context = join(root, "context");
  const destination = `${request.imageRepository}/${safeId(request.releaseId)}`;
...
function scan({ imageDigest, evidence, policyVersion }) {
  ...
  const result = run(tool("trivy"), [
    "image", "--input", join(root, "release.oci.tar"), "--scanners", "vuln,secret",
...
function sign({ imageDigest, provenanceDigest }) {
  ...
  const signed = run(tool("cosign"), [
    ...
    "--tlog-upload=false", "--yes", reference,
...
try {
  ...
  const operations = { build, registryDigest, push, generateEvidence, scan, sign };
