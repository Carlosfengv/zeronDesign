...
G8 落地时，selector patch 必须携带实时 `resourceVersion` 做 CAS；Controller 重启发现 Service 已指向
desired green 时，从 EndpointSlice/外部探测继续，而不是用旧 Store resourceVersion 覆盖。收敛或外部
探测超时时，先 CAS 切回 Store 中仍为 current 的 blue，验证 EndpointSlice 和外部 release 身份恢复，
Store 的 `currentReleaseId` 保持不变并进入 `ReconcileRequired`。详细处置见
